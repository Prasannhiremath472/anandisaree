...
exports.listAllOrdersForExport = listAllOrdersForExport;
...
async function listAllOrdersForExport(filters) {
    const { whereClause, params } = buildOrderListWhere(filters);
    return (0, db_1.query)(`SELECT o.orderNumber, o.status, o.paymentMethod, o.paymentStatus, o.totalAmount, o.createdAt,
            u.name as customerName, u.email as customerEmail, u.phone as customerPhone
     FROM \`Order\` o JOIN \`User\` u ON u.id = o.userId
     WHERE ${whereClause} ORDER BY o.createdAt DESC`, params);
}
...
async function getDashboardSummary(categoryId) {
    const now = new Date();
    ...
    const [revenueRow, orderCountRow, customerCountRow, lowStockProducts, recentOrdersRows, topProducts] = await Promise.all([
        (0, db_1.queryOne)(`SELECT SUM(totalAmount) as total FROM \`Order\` o
         WHERE createdAt >= ? AND paymentStatus = 'PAID' ${orderCategoryExists ? `AND ${orderCategoryExists}` : ""}`, categoryId ? [thirtyDaysAgo, categoryId] : [thirtyDaysAgo]),
        (0, db_1.queryOne)(`SELECT COUNT(*) as count FROM \`Order\` o WHERE createdAt >= ? ${orderCategoryExists ? `AND ${orderCategoryExists}` : ""}`, categoryId ? [thirtyDaysAgo, categoryId] : [thirtyDaysAgo]),
        ...
        (0, db_1.query)(`SELECT id FROM \`Product\` WHERE deletedAt IS NULL AND stockQuantity <= lowStockThreshold ${productCategoryExists ? `AND ${productCategoryExists}` : ""}`, categoryId ? [categoryId] : []),
        (0, db_1.query)(`SELECT o.*, u.name as user_name FROM \`Order\` o JOIN \`User\` u ON u.id = o.userId
         ...
         ORDER BY o.createdAt DESC LIMIT 5`, categoryId ? [categoryId] : []),
        (0, db_1.query)(`SELECT id, name, soldCount, sellingPrice FROM \`Product\` WHERE deletedAt IS NULL ${productCategoryExists ? `AND ${productCategoryExists}` : ""} ORDER BY soldCount DESC LIMIT 5`, categoryId ? [categoryId] : []),
    ]);
 ...
 */
async function createAdminOrder(input, createdById) {
    ...
    await (0, db_1.withTransaction)(async (conn) => {
        if (!userId && input.newCustomer) {
            ...
            if (existingByPhone) {
                ...
                userId = (0, id_1.createId)();
                const walkInEmail = email && email.length > 0 ? email : `${phone}@walkin.anandisarees.com`;
                await conn.query("INSERT INTO `User` (id, name, email, phone, role, isEmailVerified, isPhoneVerified, isActive, createdAt, updatedAt) VALUES (?, ?, ?, ?, 'CUSTOMER', 0, 0, 1, NOW(3), NOW(3))", [userId, name, walkInEmail, phone]);
                await conn.query("INSERT INTO `Wallet` (id, userId, updatedAt) VALUES (?, ?, NOW(3))", [(0, id_1.createId)(), userId]);
            }
            addressId = (0, id_1.createId)();
            await conn.query(`INSERT INTO \`Address\`
        ...
        if (!userId || !addressId) {
            throw ApiError_1.ApiError.badRequest("A customer and address are required");
        }
        const [addressRows] = await conn.query("SELECT id FROM `Address` WHERE id = ? AND userId = ?", [addressId, userId]);
        if (addressRows.length === 0)
            throw ApiError_1.ApiError.badRequest("Address not found for this customer");
        const pricedItems = await (0, checkout_service_1.priceOrderItems)(input.items);
       ...
       VALUES (?, ?, ?, ?, 'PENDING', ?, ?, ?, 0, 0, 0, ?, NOW(3), NOW(3))`, [orderId, orderNumber, userId, addressId, input.paymentMethod, paymentStatus, subtotal, totalAmount]);
        ...
        await conn.query("INSERT INTO `OrderStatusHistory` (id, orderId, status, note, changedById, createdAt) VALUES (?, ?, 'PENDING', 'Order created by admin', ?, NOW(3))", [(0, id_1.createId)(), orderId, createdById ?? null]);
